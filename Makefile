...
run: $(VENV)/bin/activate
    PLAYWRIGHT_BROWSERS_PATH=$(BROWSERS) $(PYTHON) -m streamlit run main.py --server.port 3000 --server.address 0.0.0.0
...
docker-run:
    docker run -p 3000:3000 -v $(PWD)/databases:/app/databases -v $(PWD)/reports:/app/reports jewelscope-research

docker-run-bare:
    docker run -p 3000:3000 jewelscope-research
