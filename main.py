...
# =============================================================================
# Lazy-loaded modules (imported on demand to keep startup fast)
# =============================================================================
...
def run_full_scrape(config: dict) -> dict:
    ...
    if commodity_prices:
        db.save_commodity_prices([
            {
                "commodity": a["source_name"].split("—")[-1].strip().lower().split()[0]
                           if "—" in a["source_name"] else a["source_name"],
                "price_usd": float(a["title"].split("$")[-1].replace(",", "").split()[0])
                           if "$" in a["title"] else 0,
                "unit": "oz",
                "source": a["source_url"],
            }
...
st.markdown("""
<style>
    .reportview-container { background: #0f0f12; }
    ...
    .css-145kfol { padding-top: 1.5rem; }
    ...
    .stButton button:hover { background: #3a3a5a; border-color: #7c9cff; }
    .stButton button[kind="primary"] { background: #7c9cff; color: #0f0f12; border: none; }
    .stButton button[kind="primary"]:hover { background: #6a8aee; }
</style>
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["📊 Dashboard", "🔍 Run Scan", "📄 Past Reports", "⚙️ Settings"],
